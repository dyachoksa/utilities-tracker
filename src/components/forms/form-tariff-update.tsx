...
import type { Tariff, TariffUpdateData } from "~/types/tariffs";
...
import { useUpdateTariff } from "~/hooks/use-tariff-queries";
import { useTariffType } from "~/hooks/use-tariff-type";
import { TariffUpdateSchema } from "~/schemas/tariffs";

interface Props {
  tariff: Tariff;
}

export function FormTariffUpdate({ tariff }: Props) {
  const t = useTranslations("forms");
  const { options: tariffTypeOptions } = useTariffType();

  const closeDialog = useDialogStore(useShallow((state) => state.closeDialog));
  const action = useUpdateTariff(tariff.id);

  const defaultValues = useMemo<TariffUpdateData>(
    () => ({
      name: tariff.name,
      startDate: tariff.startDate,
      tariffType: tariff.tariffType,
      isActive: tariff.isActive,
      tariffZones: tariff.tariffZones,
    }),
    [tariff]
  );

  const form = useForm<TariffUpdateData>({
  ...
  const onSubmit = form.handleSubmit((data) => {
    action.mutate(data, {
      onSuccess: () => {
        toast.success(t("tariff.update.success"), { description: t("tariff.update.successDescription") });
        closeDialog();
      },
    });
  });
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div className="flex flex-col items-center justify-between gap-4 md:flex-row-reverse">
          <Button type="submit" disabled={action.isPending}>
            {action.isPending ? <Loader2Icon className="animate-spin" /> : null} {t("tariff.update.submit")}
          </Button>
